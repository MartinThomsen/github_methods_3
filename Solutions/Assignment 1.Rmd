...
AIC()
```
    AIC: Do you get the best model at lowest cost.
    vi. which of the second-level effects explains the most variance?
    Subject as the residual standard deviation is lower, than for gender.
...
```{r}
singleModel = lm(base_pitch~gender, data = df.Subset)
```
...
3) Plotting the two-intercepts model
    ...
    Create plot
```{r}
fixedEffect = fixef(m4)
randomEffect = ranef(m4)
